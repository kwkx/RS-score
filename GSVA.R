#UTF-8 encoding, in the cluster file, "A" means "SHF" group, "B" means "LHF" group.
...
gmtFile="c5.go.v7.4.symbols.gmt"  #####we first perform GSVA of GO terms, namely the Supplementary Figure 2           
setwd("D:\\新冠文章\\代码上传\\GSVA")   #please change this path to the "GSVA" file.
...
write.table(gsvaOut, file="GOgsvaOut.txt", sep="\t", quote=F, col.names=F)
...
for(i in 1:ncol(comp)){
	...
	hmExp=data[hmGene,]
	pdf(file=paste0(contrast,"GO.heatmap.pdf"),height=6,width=10)
	pheatmap(hmExp, 
	         annotation=ann,
...
gmtFile="c2.cp.kegg.v7.2.symbols.gmt"  #####we then perform GSVA of KEGG pathways, namely the Figure 2C           
...
gsvaResult=gsva(data, 
                ...
                parallel.sz=1)
gsvaOut=rbind(id=colnames(gsvaResult), gsvaResult)
write.table(gsvaOut, file="KEGGgsvaOut.txt", sep="\t", quote=F, col.names=F)
...
for(i in 1:ncol(comp)){
  ...
  hmGene=diffTermName[1:termNum]
  hmExp=data[hmGene,]
  pdf(file=paste0(contrast,"KEGG.heatmap.pdf"),height=6,width=10)
  pheatmap(hmExp, 
           annotation=ann,
           annotation_colors = ann_colors,
           color = colorRampPalette(c(rep("blue",2), "white", rep("red",2)))(50),
